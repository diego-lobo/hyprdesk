...
- **Waybar:** full custom control BONUS - a custom module (or workspace
  ...
  `~/.config/omarchy/plugins/diego.desks/` (see the port section below).
...
- **`~/.config/hypr/hyprdesk.lua`** replaces `hyprdesk.conf`, required
  ...
  line plus one file.
- **`~/.config/omarchy/plugins/diego.desks/`** replaces the waybar
...
4. **Bar.** Whatever renders the desk strip; currently the Quickshell
   widget. `omarchy plugin list` should show `diego.desks` enabled, and
