...
`demo.gif` and `bar.png` in this directory are **placeholders**. Replace
them with real captures and the README picks them up with no edits, since
it references them by these exact names.
...
A tight crop of the Omarchy bar showing the desk indicators, with one desk
active and a couple occupied so the dimming is visible.

```bash
omarchy capture screenshot            # drag a region over the bar
```

Crop close. Roughly 800px wide and only as tall as the bar.
